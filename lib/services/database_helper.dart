...
class DatabaseHelper {
  ...
  Future _createDB(Database db, int version) async {
    ...
    ''');
    await db.insert(
        'Users', {'user_id': 1, 'name': 'Demo User', 'email': 'user@smart.com'},
  ...
  Future<List<Device>> readAllDevices() async {
    final db = await instance.database;
    final result = await db.query('Devices', where: 'building_id = ?', whereArgs: [1]);
    return result.map((json) => Device.fromMap(json)).toList();
  }

  Future<List<Recommendation>> readAllRecommendations() async {
    final db = await instance.database;
    final result = await db.query('Recommendations', where: 'building_id = ?', whereArgs: [1], orderBy: 'generated_at DESC');
